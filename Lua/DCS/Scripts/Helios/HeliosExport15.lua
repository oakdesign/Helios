...
function helios_impl.installDriver(driver)
...
function helios_impl.createModuleDriver(selfName, moduleName)
    ...
    -- execute module
    local modulePath = string.format("%sScripts\\HeliosExport\\Mods\\%s.lua", lfs.writedir(), moduleName)
    ...
    -- sanity check, make sure profile is for correct selfName, since race condition is possible
    if success and result.Name ~= selfName then
        success = false
        result = string.format("module %s is for incorrect vehicle '%s'",
            moduleName,
            result.Name
        )
    end
    ...
    -- XXX misssing:
    -- XXX FC mode
    -- XXX process input override
    -- XXX process exports override
    return driver
end
