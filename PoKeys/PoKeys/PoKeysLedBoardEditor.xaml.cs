...
{
    using GadrocsWorkshop.Helios.Interfaces.PoKeys.UndoEvents;
    ...
    /// <summary>
    /// Interaction logic for PoKeysLedBoardEditor.xaml
    /// </summary>
    public partial class PoKeysLedBoardEditor : HeliosInterfaceEditor
    {
        public PoKeysLedBoardEditor()
        {
            InitializeComponent();
        }
        ...
        public static readonly DependencyProperty GroupsViewModelProperty =
            DependencyProperty.Register("GroupsViewModel", typeof(LedGroupsViewModel), typeof(PoKeysLedBoardEditor), new PropertyMetadata(null));

        #endregion

        protected override void OnInterfaceChanged(HeliosInterface oldInterface, HeliosInterface newInterface)
        {
            PoKeysLEDBoard oldBoard = oldInterface as PoKeysLEDBoard;
            if (oldBoard != null)
            {
                oldBoard.Detach();
            }

            PoKeysLEDBoard newBoard = newInterface as PoKeysLEDBoard;
            if (newBoard != null)
            {
                newBoard.Attach();
            }

            GroupsViewModel = new LedGroupsViewModel(newInterface as PoKeysLEDBoard);
        }

        private void AddLedGroup(object sender, RoutedEventArgs e)
        {
            PoKeysLEDBoard board = Interface as PoKeysLEDBoard;
        ...
        {
            PoKeysLEDBoard board = Interface as PoKeysLEDBoard;
            board = Interface as PoKeysLEDBoard;
